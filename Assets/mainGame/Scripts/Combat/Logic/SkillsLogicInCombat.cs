...
public class SkillsLogicInCombat : MonoBehaviour
{
    ...
    public IEnumerator LowBlow(Fighter attacker, Fighter defender)
    {
        ...
        yield return movementScript.MoveSlide(attacker, Combat.GetAttackerDestinationPosition(defender, 1f));
    ...
    {
        int nShurikens = UnityEngine.Random.Range(2, 3); // 4-6 shurikens
    ...
    public IEnumerator boxCutter(Fighter attacker, Fighter defender)
    {
        int nboxCutter = UnityEngine.Random.Range(2, 3); // 4-6 shurikens

        for (int i = 0; i < nboxCutter && !Combat.isGameOver; i++)
        {
            yield return StartCoroutine(attackScript.PerformboxCutter(attacker, defender));
        }
