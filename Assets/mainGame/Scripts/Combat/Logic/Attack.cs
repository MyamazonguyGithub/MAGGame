...
public class Attack : MonoBehaviour
{
    ...
    public IEnumerator PerformShurikenFury(Fighter attacker, Fighter defender)
    {
        ...
        if (dodged)
        {
            StartCoroutine(Combat.movementScript.RotateObjectOverTime(shurikenInstance, new Vector3(0, 0, 3000), 1.0f));
            ...
            yield return DefenderDodgesAttack(defender);
            yield return new WaitForSeconds(GeneralUtils.GetRealOrSimulationTime(1.0f)); //Wait for the shuriken to be in its final position before destroying it (This could be avoided with colliders)
            Destroy(shurikenInstance);
            yield break;
        }

        StartCoroutine(Combat.movementScript.RotateObjectOverTime(shurikenInstance, new Vector3(0, 0, 500), 1.0f));
        yield return StartCoroutine(Combat.movementScript.MoveShuriken(shurikenInstance, shurikenStartPos, shurikenEndPos, 1.0f));
        Destroy(shurikenInstance);
    ...
    public IEnumerator PerformboxCutter(Fighter attacker, Fighter defender)
    {
        bool dodged = IsAttackDodged(defender);

        Vector3 boxCutterStartPos = attacker.transform.position;
        Vector3 boxCutterEndPos = defender.transform.position;
        boxCutterStartPos.y -= 0.4f;
        boxCutterEndPos.y -= 0.4f;
        boxCutterEndPos.x = GetboxCutterEndPositionX(dodged, attacker, boxCutterEndPos);

        FighterAnimations.ChangeAnimation(attacker, FighterAnimations.AnimationNames.THROW);
        yield return new WaitForSeconds(GeneralUtils.GetRealOrSimulationTime(.1f)); //Throw the boxCutter when the fighter arm is already up

        GameObject boxCutterInstance = Instantiate(boxCutter, boxCutterStartPos, Quaternion.identity);

        if (dodged)
        {
            StartCoroutine(Combat.movementScript.RotateObjectOverTime(boxCutterInstance, new Vector3(0, 0, 3000), 1.0f));
            StartCoroutine(Combat.movementScript.MoveboxCutter(boxCutterInstance, boxCutterStartPos, boxCutterEndPos, 0.5f)); //We dont yield here so we can jump mid animation
            yield return new WaitForSeconds(GeneralUtils.GetRealOrSimulationTime(.2f)); //Wait for the boxCutter to approach before jumping
            yield return DefenderDodgesAttack(defender);
            yield return new WaitForSeconds(GeneralUtils.GetRealOrSimulationTime(1.0f)); //Wait for the boxCutter to be in its final position before destroying it (This could be avoided with colliders)
            Destroy(boxCutterInstance);
            yield break;
        }

        StartCoroutine(Combat.movementScript.RotateObjectOverTime(boxCutterInstance, new Vector3(0, 0, 500), 1.0f));
        yield return StartCoroutine(Combat.movementScript.MoveboxCutter(boxCutterInstance, boxCutterStartPos, boxCutterEndPos, 1.0f));
        Destroy(boxCutterInstance);
        ...
        yield return DefenderReceivesAttack(attacker, defender, attacker.damage * GlobalConstants.SkillDamages.boxCutter, 0.25f, 0);
    }
    ...
    private float GetboxCutterEndPositionX(bool dodged, Fighter attacker, Vector3 boxCutterEndPos)
    {
        if (dodged) return Combat.player == attacker ? boxCutterEndPos.x + 10 : boxCutterEndPos.x - 10;
        //To move the hitbox a bit upfront
        return Combat.player == attacker ? boxCutterEndPos.x - 1f : boxCutterEndPos.x + 1f;
    }
